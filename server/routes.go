...
func GenerateHandler(c *gin.Context) {
	...
	case req.Prompt != "":
		...
		p, err := Prompt(model.Template, req.Prompt, req.System, "", true)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
	...
	go func() {
		...
		fn := func(r llm.PredictResult) {
			...
			if r.Done {
				...
				if !req.Raw {
					p, err := Prompt(model.Template, req.System, req.Prompt, generated.String(), false)
					slog.Debug("generate handler finished", "prompt", p)
					if err != nil {
						c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
						return
					}
					...
					slog.Debug("generate handler finished", "tokens", tokens)
